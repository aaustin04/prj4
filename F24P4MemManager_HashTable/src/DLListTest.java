...
/**
 * @author Eric lifted from CS 2114 source
 * @author maellis1
 * @version 11-2-15
 */
public class DLListTest extends TestCase {
    ...
    /**
     * Tests the add to end method in DLL.
     */
    public void testaddToEnd() {
        assertEquals(0, list.size());
        assertFalse(list.contains("A"));
        list.addToEnd("A");
        assertEquals(list.getTail().getIndex(), list.getHead().getIndex());
        assertEquals(1, list.size());
        assertTrue(list.contains("A"));
        assertEquals(list.getHead().getIndex(), 0);
        assertFalse(list.contains("B"));
        list.addToEnd("B");
        assertTrue(list.contains("B"));
        assertEquals(2, list.size());
